...
class event_detail extends Component {
  ...
  componentDidMount() {
    ...
    faxios()
      .get("/events/list/2022/")
      .then((d) => {
        let data = d.data.data;
        let event_detail = data.find(
          (event) => event.id === Number(this.event_id)
        );
  ...
  _event_register = (e) => {
    ...
    if (this.props.auth.loggedin) {
      if (this.props.auth.verified) {
        faxios()
          .post(`/events/register/${this.props.match.params.id}/`)
          .then((res) => {
            console.log(res);
        ...
        alert(
          "Please verify your phone number before registration. To do the same click on your name."
        );
        this.setState({
          btnloader: false,
        });
  ...
  _event_unregister = (e) => {
    ...
    faxios()
      .post(`/events/unregister/${this.props.match.params.id}/`)
      .then((res) => {
        let total_registrations = this.state.people_registered;
        this.setState({
          register: false,
      ...
      event_detail = (
        <div className="event_detail container">
          <div className="event-item1">
            <img
              className="shadow-lg bg-white rounded event-detail-pic"
              width="400"
              height="400"
              alt={event.name}
              src={event.icon}
          ...
          <div className="event-item2">
            <div className="event-content">
              ...
              <div className="event-detail-name">
                <h4 className="e-name shadow p-3 mb-5 bg-white rounded">
                  {event.name}
                </h4>
              </div>
              <br></br>
              <div className="event-venue" style={{ color: "grey" }}>
                <i className="fas fa-map-marker-alt"></i>&nbsp;Venue:&nbsp;
                <span style={{ color: "white" }}>{event.venue}</span>
              </div>
              <br></br>
              <div className="event-time" style={{ color: "grey" }}>
                <span>
              ...
              <div className="event-email">
                ...
                <a className="e-email" href={`mailto:${event.email}`}>
                  {event.email}
                </a>
              </div>
              <div
                className="registered"
                style={{ color: "grey", fontWeight: "800", marginTop: "15px" }}
              >
                <i class="fas fa-user-friends"></i>&nbsp; Total Registered :{" "}
                <span style={{ color: "white", fontSize: "40px" }}>
                  {this.state.people_registered}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="events">
          ...
          <div className="event-detail-page">
            ...
            <div className="button-div">
              {this.state.register && this.props.auth.loggedin ? (
                <button
                  onClick={this._event_unregister}
                  className="btn unregister-btn"
                >
                  {this.state.btnloader ? <BtnLoader /> : "Unregister"}
                </button>
              ) : (
                <button
                  onClick={this._event_register}
                  className="btn register-btn"
                >
                  {this.state.btnloader ? <BtnLoader /> : "Register"}
                </button>
              )}
            </div>
          </div>
