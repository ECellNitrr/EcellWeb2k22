...
startups[appr_state].idea_approved= true
...
const appr_state= startups.findIndex(temp => temp.id === user.id)
console.log(appr_state)
startups[appr_state].idea_approved= false
this.setState(startups)
})
            ...
            <div class="card-body">
                ...
                <div dangerouslySetInnerHTML={{ __html: user.describe_idea }}></div>
                 ...
                 <div>
                    ...
                    {user.idea_approved ? <button onClick={()=>this._dis_approve(user)} className='btn float-right font-weight-bold btn-danger'>Disapprove  Idea</button> : <button onClick={()=>this._approve(user)} className='btn float-right font-weight-bold btn-success'>Approve Idea</button>}
